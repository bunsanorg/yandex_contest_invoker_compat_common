...
struct ContextFixture {
  ...
  void verifyPG(const std::size_t i) {
    ...
    pg[i].context();
    for (std::size_t j = 0; j < pSize; ++j) verifyP(i, j);
  }
  ...
  void verifyNoPG(const std::size_t i) {
    for (std::size_t j = 0; j < pSize; ++j) verifyNoP(i, j);
  }
...
BOOST_AUTO_TEST_CASE(ProcessGroupHandle__destroy) {
  pg[0].destroy();
  verifyNoPG(0);
  for (std::size_t i = 1; i < pgSize; ++i) verifyPG(i);
}

BOOST_AUTO_TEST_CASE(ProcessHandle__destroy) {
  p[0][0].destroy();
  verifyNoP(0, 0);
  for (std::size_t j = 1; j < pSize; ++j) verifyP(0, j);
